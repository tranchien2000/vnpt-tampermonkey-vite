import { FirebaseService } from "/src/api/firebaseService.js.js";
    ...
    if (user) {
      ...
      document.getElementById('vnpt-btn-cloud-push-all').onclick = async () => {
        try {
          ...
          const { getProfiles } = await import("/src/features/profileManager.js.js");
          const { Storage } = await import("/src/utils/storage.js.js");
          const { 
              SK_CALC_MAP, SK_HOTKEYS, SK_GEMINI_KEY,
              LOCAL_KEY_FIELDS, SK_TEMPLATES, SK_TAX, 
              SK_DATA_DEF, LOCAL_KEY_DEFAULT_FIELDS,
              SK_ADDRESS_LEARNING
          } = await import("/src/core/constants.js.js");
          const { DEFAULT_CALC_MAP } = await import("/src/core/defaults.js.js");
        ...
        try {
          ...
          if (confirm(`Tìm thấy dữ liệu đồng bộ. Bạn có muốn ghi đè bộ cài đặt Local hiện tại không?`)) {
             ...
             if (cloudProfiles.length > 0) {
                const { importProfiles } = await import("/src/features/profileManager.js.js");
             ...
             try {
                ...
                const { loadSavedData } = await import("/src/features/fieldsManager.js.js");
                loadSavedData(); 
                
                // Nạp lại các biến mặc định cho Data Fill (Nếu đang mở tab này)
                const { initSyncEngine } = await import("/src/features/dataFill/syncEngine.js.js");
                initSyncEngine(); 
